...
export default function NotFoundComponent({ title, description }: props) {
  return (
    <main className="min-h-[60vh] max-w-7xl mx-auto md:px-16 px-6">
      {/* align items center */}
      <header className="max-w flex flex-col items-center">
        <Image
          width={80}
          height={80}
          src={duckImage}
        ...
        <p className="max-w text-base dark:text-zinc-400 text-zinc-600 leading-relaxed">
          {description}
        </p>
      </header>

      <div className="max-w grid lg:grid-cols-2 grid-cols-1 gap-4 mt-12">
        <FeaturedPosts />
      </div>
    </main>
  );
